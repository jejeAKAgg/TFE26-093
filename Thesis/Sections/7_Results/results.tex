...
The results of Tournament 1 are unambiguous. MAST, UCT, and FlatMC 
all achieve dramatically higher win rates against the Random agent, 
while the Random agent performs at 0.5\% across all its matchups. 
MAST dominates the tournament with 79.8\%, followed by UCT at 43.8\% 
and FlatMC at 25.8\%.
...
\subsection{The Role of Simulation Depth}

An additional observation concerns the relative performance of MAST, 
UCT, and FlatMC within Tournament 1. Since all three agents 
effectively play with full information, the differences between them 
reflect the depth and quality of their search rather than any 
hidden-information reasoning ability. MAST's move-average statistics 
allow it to efficiently sequence shots toward historically rewarded 
cells, producing the strongest performance. UCT builds a search tree 
but its tree construction overhead reduces its effective exploitation 
of perfect information relative to MAST. FlatMC, which evaluates 
each action independently through flat rollouts, exploits the 
available information least efficiently of the three.
...
The first and most important observation from Tournament 2 is that 
the artificial performance advantage observed in Tournament 1 is 
eliminated. UCT+Det, MAST+Det, and FlatMC+Det no longer dominate 
the Random agent by a large margin. The win rates of the determinized 
agents against Random converge toward the 50\% region, consistent 
with agents that are now reasoning about hypothetical worlds rather 
than exploiting the true hidden state. OSLA+Det emerges as the 
strongest agent with a win rate of 87.5\%, closely mirroring its 
performance in the TAG benchmark and confirming that the 
determinization layer provides a genuine and exploitable signal even 
for shallow one-step search.
...
MAST+Det and OSLA+Det. However, it likely reduces the effective 
simulation count relative to Tournament 1, which may partly explain 
the modest performance of UCT+Det compared to its native counterpart.
...
The most fundamental prediction from Chapter~\ref{BattleshipInTAG} is that simulation-based agents given access to incoherent or trivially correct hidden states should not outperform the Random agent. Under Random Determinization in TAG, all agents converged to approximately 50\% win rates. Under native Ludii without the determinization layer, agents converge to near-perfect win rates against Random, which is the opposite of chance: they are not being given incoherent information, they are being given perfect information.

This asymmetry highlights that the Ludii native condition is not a form of Random Determinization but rather a form of omniscient play. Tournament 1 therefore does not replicate the TAG Random Determinization condition; it represents a qualitatively different and more severe failure mode. The convergence to chance that TAG's Random Determinization demonstrated is instead replicated by Tournament 2, where determinized agents lose their information advantage and performance returns toward the 50\% region, consistent with the TAG baseline.

\subsection{Pattern 2: Recovery Under Constraint-Based Determinization}
Chapter~\ref{BattleshipInTAG} showed that introducing constraint-based determinization produced a dramatic improvement for all simulation-based agents, with OSLA and RMHC reaching win rates well above 50\% against Random. 
Tournament 2 reproduces this pattern directly. OSLA+Det achieves 87.5\%, consistent with the 87\% observed in TAG under Constraint-Based Determinization, confirming that the behavioral recovery under informed determinization is not framework-specific but reflects a general property of simulation-based search under hidden information. The re-implementation of OSLA in Ludii therefore serves as a direct 
...
Across all conditions in both TAG and Ludii, the Random agent is the one invariant. Because it never calls a simulation copy method, its behavior is entirely unaffected by what the copy mechanism returns. Its win rate should remain close to 50\% in all conditions where its opponents are reasoning under genuine uncertainty, and should drop substantially below 50\% only when its opponents are reasoning with perfect information.

This prediction holds across all four tournaments: TAG Random Determinization (Random $\approx$ 44\%), TAG Constraint-Based (Random $\approx$ 5\% against strong agents), TAG Heatmap-Guided (similar), Ludii Tournament with Native Agents (Random $\ll$ 50\%), and Ludii Tournament with Determinized Agents (Random $\approx$ 50\%). The consistent behavior of the Random agent across frameworks and conditions provides strong evidence that the effects observed are due to the determinization mechanism rather than to confounding factors.
